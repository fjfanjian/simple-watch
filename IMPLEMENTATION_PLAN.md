...
当前交付边界必须明确：截至本版导出，仓库中交付的是实施计划，并不表示文中列出的源码、Compose、Caddyfile或脚本已经生成。本文的目标是让后续实现工作不再重新做架构选择；只有完成第18章各阶段、生成实际文件并通过第19章清单后，仓库才是可直接部署运行的项目。不能把“计划足以指导实施”误读为“现在已有代码可执行”。
